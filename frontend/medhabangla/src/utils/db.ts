...
// Define the database
export class MedhaBanglaDB extends Dexie {
...
export const db = new MedhaBanglaDB();
 ...
 * Mark note as synced
 */
export async function markNoteSynced(id: number, serverId: number): Promise<void> {
 ...
 * Get cached quizzes
 */
export async function getCachedQuizzes(subject?: string, classTarget?: number): Promise<CachedQuiz[]> {
 ...
 */
export async function saveQuizAttemptOffline(attempt: Omit<QuizAttempt, 'id'>): Promise<number> {
 ...
 */
export async function saveStudySessionOffline(session: Omit<StudySession, 'id'>): Promise<number> {
 ...
 */
export async function markStudySessionSynced(id: number, serverId: number): Promise<void> {
 ...
 * Get cached books
 */
export async function getCachedBooks(classLevel?: number): Promise<CachedBook[]> {
 ...
 */
export async function saveBookmarkOffline(bookmark: Omit<Bookmark, 'id'>): Promise<number> {
 ...
 */
export async function getBookmarkForBook(bookId: number): Promise<Bookmark | undefined> {
 ...
 */
export async function markBookmarkSynced(id: number, serverId: number): Promise<void> {
 ...
 */
export async function syncAllData(apiBaseUrl: string, token: string): Promise<{
    success: boolean;
    synced: {
        notes: number;
        quizAttempts: number;
    ...
    try {
        ...
        for (const session of unsyncedSessions) {
            try {
                const response = await fetch(`${apiBaseUrl}/api/accounts/study-sessions/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Token ${token}`
