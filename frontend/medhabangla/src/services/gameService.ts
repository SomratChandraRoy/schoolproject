...
class GameService {
    ...
    // Player Profile
    async createProfile(playerName?: string, grade?: number): Promise<PlayerProfile> {
    ...
    // Game Session
    async startSession(gameType: string): Promise<{ session: GameSession; is_new: boolean }> {
    ...
    async getGameLeaderboard(gameType: string, limit: number = 10): Promise<LeaderboardEntry[]> {
    ...
    async getGradeLeaderboard(grade: number, limit: number = 10): Promise<LeaderboardEntry[]> {
...
export type { GameData, PlayerProfile, GameSession, ScoreSubmission, LeaderboardEntry };
