    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data.subjects) && data.subjects.length > 0) {
          ...
          const fallbackSubjects = getCurriculumSubjectsForClass(
            classLevel,
          ).map((subject, index) => ({
            id: index + 1,
            name: subject.name,
        ...
        const fallbackSubjects = getCurriculumSubjectsForClass(classLevel).map(
          (subject, index) => ({
            id: index + 1,
            name: subject.name,
            bengali_title: subject.bengaliName,
    ...
    if (adaptiveMode) {
      navigate("/quiz/adaptive", {
        state: {
          ...
          questionType: selectedQuestionTypes[0] || "mcq", // Backward-compatible fallback
        },
      });
    } else {
      navigate("/quiz", {
      ...
      <div className="max-w-7xl mx-auto py-8 sm:py-12 px-3 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Select subjects and difficulty level to begin your personalized quiz
            experience
          </p>
        ...
        {loading ? (
          ...
          <>
            <div className="bg-white/95 dark:bg-gray-800 rounded-3xl shadow-xl p-5 sm:p-8 mb-8 border border-sky-100 dark:border-gray-700">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
                📚 Select Subjects
              </h2>
              <p className="text-gray-600 dark:text-gray-300 mb-6">
                Choose one or more subjects for your quiz (showing subjects for
                Class {userClass})
              </p>
                    ...
                    {subjects
                      ...
                      .map((subject) => (
                        <button
                          ...
                          className={`p-5 sm:p-6 rounded-2xl border-2 transition-all transform hover:scale-105 min-h-[120px] ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {difficulties.map((difficulty) => (
                  <button
                    ...
                    className={`p-5 sm:p-6 rounded-2xl border-2 transition-all min-h-[120px] ${
            ...
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-3xl shadow-xl p-5 sm:p-8 mb-8 border-2 border-purple-200 dark:border-purple-800">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-gray-700 dark:text-gray-300 mb-4">
                    Experience intelligent, progressive learning with AI-powered
                    questions that adapt to your performance!
                  </p>
                  <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-green-500">✓</span>
                      <span>
                        At 90% completion, AI generates personalized questions
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-green-500">✓</span>
                      <span>
                        Difficulty increases based on your performance
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-green-500">✓</span>
                  ...
                  {adaptiveMode && (
                    <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                      <p className="text-sm text-yellow-800 dark:text-yellow-200">
                        ℹ️ <strong>Note:</strong> Adaptive mode uses a single
                        subject and supports your selected question types
                        (MCQ/Short/Long). Difficulty adjusts automatically.
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <button
                    ...
                    <span
                      className={`inline-block h-10 w-10 transform rounded-full bg-white shadow-lg transition-transform ${
                        adaptiveMode ? "translate-x-12" : "translate-x-1"
                      }`}
                    />
                  </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {questionTypes.map((type) => (
                  <button
                    ...
                    className={`p-5 sm:p-6 rounded-2xl border-2 transition-all transform hover:scale-105 min-h-[140px] ${
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      <span className="font-semibold">Tip:</span> You can select
                      multiple question types to practice different formats in
                      one quiz!
                    </p>
                  </div>
                </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3 sm:space-x-0">
          <Link
            to="/dashboard"
            className="px-6 py-3 min-h-[48px] border border-gray-300 dark:border-gray-600 rounded-xl font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-center">
            ← Back
          </Link>
          <button
            onClick={handleStartQuiz}
            disabled={selectedSubjects.length === 0}
            className={`px-8 py-3 min-h-[48px] rounded-xl font-bold text-white transition-all ${
