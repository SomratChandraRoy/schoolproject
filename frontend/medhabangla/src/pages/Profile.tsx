...
const Profile: React.FC = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Show premium modal after data is loaded
    ...
    if (type === "fav") {
      if (formData.favSubjects.includes(subjectCode)) {
        ...
        setFormData({
          ...
          dislikedSubjects: formData.dislikedSubjects.filter(
            (s) => s !== subjectCode,
          ),
        });
      }
  ...
  const recentActivity = [
    ...
    {
      id: 2,
      activity: "Played Memory Matrix",
      points: "+30",
      time: "1 day ago",
    },
    {
      id: 3,
      activity: "Read Physics Chapter 3",
      points: "+20",
      time: "2 days ago",
    },
                  ...
                  <div className="md:ml-8 mt-6 md:mt-0 text-center md:text-left">
                    ...
                    <div className="mt-4 flex flex-wrap justify-center md:justify-start gap-4">
                      ...
                      <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                        ...
                        <p className="text-xl font-bold text-white">
                          {getPrimaryRoleLabel(userData)}
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                {/* View Premium Profile Button */}
                <button
                  onClick={() => setShowPremiumModal(true)}
                  className="mt-6 md:mt-0 px-6 py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white font-semibold rounded-xl border-2 border-white/30 transition-all duration-300 hover:scale-105 shadow-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          {subjects.map((subject) => (
                            <div
                              ...
                              <input
                                type="checkbox"
                                id={`fav-${subject.code}`}
                                checked={formData.favSubjects.includes(
                                  subject.code,
                                )}
                                onChange={() =>
                                  handleSubjectToggle(subject.code, "fav")
                                }
                                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                              />
                              <label
                                htmlFor={`fav-${subject.code}`}
                    ...
                    <div>
                      ...
                      {editing ? (
                          ...
                          {subjects.map((subject) => (
                            <div
                              ...
                              <input
                                ...
                                checked={formData.dislikedSubjects.includes(
                                  subject.code,
                                )}
                                onChange={() =>
                                  handleSubjectToggle(subject.code, "disliked")
                                }
                                className="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded"
                              />
                              <label
                                htmlFor={`disliked-${subject.code}`}
                                className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                                {subject.bengaliName} ({subject.name})
                              </label>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {userData.disliked_subjects?.map(
                            (subject: string) => (
                              <span
                                key={subject}
                                className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-100">
                                {getSubjectNameByCode(
                                  userData.class_level,
                                  subject,
                                )}
