...
const GameBoard: React.FC<GameBoardProps> = ({ level, onGameComplete }) => {
  ...
  const handleCellClick = (cellId: number) => {
    ...
    if (newUserPattern.length === pattern.length) {
      const rawTimeTaken = startTime ? (Date.now() - startTime) / 1000 : 0;
      const timeTaken = Math.max(rawTimeTaken, 1);
      const score = Math.max(
        100,
        Math.round(1000 * (patternLength / timeTaken)),
      );
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      ...
      <div className="text-center text-sm text-gray-600 dark:text-gray-400">
        <p>
          Pattern Length: {patternLength} | Grid: {cols}x
          {Math.ceil(gridSize / cols)}
        </p>
        {isUserTurn && (
