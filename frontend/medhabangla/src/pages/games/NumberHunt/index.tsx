  ...
  const initializeGame = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Profile fetch failed for Number Hunt:", profileError);
      }

      setLoading(false);
    } catch (error) {
      console.error("Error initializing game:", error);
      setError("Failed to initialize Number Hunt. Please try again.");
      setLoading(false);
    }
  };
  ...
  const handleGameComplete = async (
    ...
    try {
      const accuracy = success
        ? Math.max(0, Math.min(100, 100 - mistakes * 10))
        : 0;
