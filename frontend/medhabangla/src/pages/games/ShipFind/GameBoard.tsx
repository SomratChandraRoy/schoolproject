...
interface GameBoardProps {
    level: number;
    onGameComplete: (success: boolean, score: number, clicks: number, timeInSeconds: number) => void;
}
...
const GameBoard: React.FC<GameBoardProps> = ({ level, onGameComplete }) => {
        ...
        for (let i = 0; i < shipLengths.length; i++) {
            ...
            while (!placed && attempts < 100) {
                ...
                if (canPlaceShip(newGrid, row, col, length, horizontal)) {
                    const positions = placeShip(newGrid, row, col, length, horizontal, i + 1);
                    placedShips.push({
                        id: i + 1,
                        length,
                        positions,
    ...
    const handleCellClick = (row: number, col: number) => {
        ...
        if (cell.hasShip) {
            ...
            if (allShipsFound) {
                setGameOver(true);
                const timeInSeconds = startTime ? Math.floor((Date.now() - startTime) / 1000) : 0;
                const score = calculateScore(timeInSeconds);
                setTimeout(() => {
                    onGameComplete(true, score, clicks + 1, timeInSeconds);
                }, 500);
            }
        } else {
            // Miss
    ...
    const getCellClass = (cell: Cell): string => {
        const baseClass = 'aspect-square rounded-lg transition-all duration-200 cursor-pointer hover:scale-105';
        ...
        <div className="w-full max-w-4xl mx-auto p-4">
            ...
            <div className="mb-6 grid grid-cols-3 gap-4 text-center">
                <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
                    <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{clicks}</div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">Ships Found</div>
                </div>
            </div>
            ...
            <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                <h3 className="font-bold text-blue-800 dark:text-blue-200 mb-2">How to Play:</h3>
