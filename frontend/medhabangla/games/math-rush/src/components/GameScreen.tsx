...
export function GameScreen({ onFinish }: GameScreenProps) {
  ...
  const nextProblem = useCallback((wasCorrect: boolean) => {
  ...
  const handleStop = () => {
    ...
    const finalStats = {
      ...stats,
      averageTime: stats.problemTimes.length > 0
        ? stats.problemTimes.reduce((a, b) => a + b, 0) / stats.problemTimes.length
        : 0,
    }
    onFinish(finalStats)
  }
  ...
  return (
    <div className="relative min-h-dvh flex flex-col items-center p-4 pb-6 overflow-hidden">
      ...
      >
        ...
        <span className="text-[10px] text-muted-foreground font-semibold ml-1">Lv.{difficulty + 1}</span>
      </motion.div>

      {/* Timer */}
      <div className="relative z-10 mb-5">
        <CircularTimer timeLeft={timeLeft} totalTime={problem.timeLimit} size={100} />
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="bg-card rounded-3xl border border-border shadow-lg p-6 text-center">
            ...
            {/* Answer display */}
            <div className={`
              relative h-16 flex items-center justify-center rounded-2xl border-2 transition-colors duration-200
              ${feedback === 'correct' ? 'border-emerald-400 bg-emerald-50' :
                feedback === 'wrong' || feedback === 'timeout' ? 'border-red-400 bg-red-50 animate-shake' :
            ...
            `}>
              <input
                ...
                placeholder="?"
                className="w-full h-full bg-transparent text-center font-heading text-3xl font-bold
            ...
            <AnimatePresence>
              {feedback && (
                ...
                >
                  ...
                  {feedback === 'timeout' && `⏰ Time's up! Answer: ${problem.answer}`}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
